...
class FragMarketSettings : PreferenceFragmentCompat() {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        // Setup dark mode preference listener
        val darkModePreference: DropDownPreference? = findPreference("pref_dark_mode")
        darkModePreference?.onPreferenceChangeListener = Preference.OnPreferenceChangeListener { _, newValue ->
            context?.let { ctx ->
                ThemeManager.setDarkMode(ctx, newValue as String)
            }
            true
        }
    }
